...
# No default path: this repo is a public fork and must not carry anyone's
# host layout. Pass the log as an argument or set INCIPIT_PLEX_LOG. Typical
# locations are "<plex-data-dir>/Logs/PMS Plugin Logs/com.plexapp.agents.incipit.log".
LOG="${1:-${INCIPIT_PLEX_LOG:-}}"
if [ -z "$LOG" ]; then
	echo "FAIL: no plugin log given." >&2
	echo "  usage: scripts/check_deploy_banner.sh <path-to-com.plexapp.agents.incipit.log>" >&2
	echo "  or:    INCIPIT_PLEX_LOG=<path> scripts/check_deploy_banner.sh" >&2
	exit 2
fi
